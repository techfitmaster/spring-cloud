...
public class Demo {
    ...
    /**
     * 模拟数据预处理 1. 确定数据类型 id, 类别，数值 id: 校验重复 数值： 校验是否是number 类别：
     * <p>
     * <p>
     * 2. 校验数据库为空
     * <p>
    ...
    private static void preprocessingData() {
        ...
        StructType schema = new StructType(
                new StructField[] { new StructField("id", DataTypes.DoubleType, false, Metadata.empty()),
        ...
        Dataset<Row> dataset = spark.read().option("header", true).schema(schema).csv(filepath);
        dataset.show();
        dataset.printSchema();
        ...
        JavaRDD<Row> javaRDD =  transform.toJavaRDD();
